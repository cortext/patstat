...
drop table if exists patstatAvr2017_lab.`geo_ura`;
CREATE TABLE patstatAvr2017_lab.`geo_ura` (
  ...
  `sourcename` varchar(50) character set utf8 collate utf8_unicode_ci default null,
...
load data local infile
	'/home/luisd/tmp/ura_classification_201910.txt'
into table patstatAvr2017_lab.geo_ura
character set utf8
...
(@FID, @name, @type, @ctrycodeis, @nbcities, @sumpop, @source, @sourceid, @sourcename, @sqkm, @longitude, @latitude, @uniqueid)
set
    name = nullif(@name, ''),
    type = nullif(@type, ''),
